...
import compositegrid.coordinates.CoordinateUtils;
...
public abstract class AbstractGridRenderer<H, V, E> {
    ...
    protected abstract void drawHexagon(Graphics2D g2d, HexagonCoordinate coordinate, HexagonCell<H, V, E> cell, Shape hexagonShape, Point2D vertexPosition);
    /** Draws a edge at the provided coordinate. E will be null it out of bounds   */
    protected abstract void drawEdge(Graphics2D g2d, TriangleCoordinate coordinate, E edge, TriangleDirection direction, Point2D lineStart, Point2D lineFinish);
    /** Draws a hexagon edge at the provided coordinate. E will be null it out of bounds   */
    protected abstract void drawHexagonBorderEdge(Graphics2D g2d, TriangleCoordinate coordinate, E edge, TriangleDirection direction, Point2D lineStart, Point2D lineFinish);
    /** Draws a vertice at the provided coordinate. TriangleCell will be null it out of bounds   */
    protected abstract void drawVertices(Graphics2D g2d, TriangleCoordinate coordinate, TriangleCell<V, E> cell, Point2D vertexPosition);
    /** Draws the vertice at the hexagon centre of the provided coordinate. TriangleCell will be null it out of bounds   */
    protected abstract void drawHexagonCentreVertice(Graphics2D g2d, TriangleCoordinate coordinate, TriangleCell<V,E> cell, Point2D vertexPosition);
            ...
            for (int y = minY; y < maxY; y++) {
                for (int x = minX; x < maxX; x++) {
                    ...
                    switch (drawingOperation) {
                        case 0:
                            prepareHexagonDraw(g2d, triCoordinate, vertexPosition);
                            break;
                        case 1:
                            prepareDrawEdges(g2d, triCoordinate, triCell, vertexPosition);
                            break;
                        case 2:
                            drawVertices(g2d, triCoordinate, triCell, vertexPosition);
                            break;
                        case 3:
                            if (CoordinateUtils.fallsOnHexagonCentre(triCoordinate))
                                drawHexagonCentreVertice(g2d, triCoordinate, triCell, vertexPosition);
                            break;
                    }
                }
            }
    ...
    public Point2D getCameraPixelOffset() {
        Point2D offset = geometry.screenPosition(cameraPosition);
        return new Point2D.Double(
                offset.getX() - (canvasWidthPixels / 2.0),
                offset.getY() - (canvasHeightPixels / 2.0)
        );
    }
    ...
    private void prepareHexagonDraw(Graphics2D g2d, TriangleCoordinate triCoord, Point2D vertexPosition) {
        ...
        if (CoordinateUtils.fallsOnHexagonCentre(triCoord)) {
            ...
            drawHexagon(g2d, hexCoordinate, hexCell, constructHexagon(hexCoordinate), vertexPosition);
        }
    }

    private void prepareDrawEdges(Graphics2D g2d, TriangleCoordinate coordinate, TriangleCell<V, E> cell, Point2D vertexPosition) {
        //only draw 3 edges
        for (int i = 0; i < 3; i++) {
            ...
            drawEdge(g2d, coordinate, edge, direction, vertexPosition, lineFinish);
            ...
            if (CoordinateUtils.fallsOnHexagonBorder(coordinate, direction))
                drawHexagonBorderEdge(g2d, coordinate, edge, direction, vertexPosition, lineFinish);
        }
    }
